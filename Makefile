...
jslint:
	if type jslint >/dev/null 2>&1 ; then jslint sample/static/js/*; else echo "jslint not installed. To install node and jshint, use 'make install_jslint"; fi
...
coverage:
	coverage erase
	coverage run --source sample setup.py test
	coverage report -m
	coverage html
